...
var require_chart = __commonJS({
  "node_modules/chartjs/chart.js"(exports, module) {
    ...
    (function() {
      var Helpers = {
        ...
        lowerMin: function(arr) {
          ...
          if (Array.isArray(v))
            v = Helpers.lowerMin(v);
          for (var index = 1; index < arr.length; ++index) {
            var v2 = arr[index];
            if (v2 == null)
              continue;
            if (Array.isArray(v2))
              v2 = Helpers.lowerMin(v2);
            if (v2 < v)
              v = v2;
          }
      ...
      var BarChart = function() {
        function BarChart2(ctx, options) {
          this.mouseListeners = [];
        ...
        BarChart2.prototype.update = function(content) {
          ...
          for (var i = 0; i < content.data.length; ++i) {
            ...
            if (this.options.scaleStyle === "log2") {
              if (isArr) {
                for (var i3 = 0; i3 < content.data[i].length; ++i3)
                  content.data[i][i3] = Math.log2(content.data[i][i3]);
              } else
                content.data[i] = Math.log2(content.data[i]);
            }
            if (isArr) {
              var mean = Helpers.avg(content.data[i]);
              var acc = 0;
              for (var i2 = 0; i2 < content.data[i].length; ++i2)
                acc += Math.pow(mean - content.data[i][i2], 2);
        ...
        BarChart2.prototype.redraw = function() {
          setTimeout((function() {
            this._draw();
          }).bind(this), 0);
        };
        BarChart2.prototype.mousemove = function(x, y) {
          var res = null;
          for (var index = 0; index < this.mouseListeners.length; ++index) {
            if (res = this.mouseListeners[index](x, y))
              break;
          }
        ...
        BarChart2.prototype._draw = function() {
          var labelPositions = {};
          this.mouseListeners = [];
          this.fillRegions = [];
          ...
          if (options.barStyle === "stacked") {
            ...
            for (var cmIndex = 0; cmIndex < content.data.length; ++cmIndex) {
              ...
              if (Array.isArray(doB = content.data[cmIndex])) {
                var tempSum = 0;
                for (var ii2 = 0; ii2 < doB.length; ++ii2)
                  tempSum += doB[ii2];
          ...
          if (options.scaleStyle.indexOf("adaptive") === 0) {
            if (options.scaleStyle.indexOf(":") !== -1) {
              ...
              maxChartValue *= 1 + (1 - floater) / 2;
            }
          } else
            minChartValue = 0;
          if (options.defaultMaxTick > maxChartValue)
            maxChartValue = options.defaultMaxTick;
          ...
          if (options.tickFormatterMeasure != null)
            maxYAxisTickWidth = options.tickFormatterMeasure;
          ...
          for (index = 0; index < content.labels.length; ++index) {
            ...
            if (Array.isArray(tLabel)) {
              ...
              maxTextWidth = Math.max(maxTextWidth, ctx.measureText(tLabel).width);
          }
          ...
          for (index = 0; index < content.labels.length; ++index) {
            ...
            if (xLabelsRotated) {
              ...
              y = [x, x = -y][0];
              if (y < lastLabelY + options.fontSizeLabels)
                continue;
              lastLabelY = y;
            }
            ...
            if (Array.isArray(cLabel)) {
              ...
              if (xLabelsRotated)
                yUp = -options.fontSizeLabels * 0.25;
          ...
          if (content.topLabels != null)
            ctx.lineTo(leftXPadding + remainingWidth, topYPadding);
          ctx.stroke();
          ctx.restore();
          ...
          for (index = 0; index < ticks.length; ++index) {
            ...
            if (tickHeight < 0)
              continue;
            if (options.scaleStyle == "log2" && ticks[index] !== 0)
              ticks[index] = Math.round(Math.pow(2, ticks[index]));
            else
              ticks[index] = Math.floor(ticks[index] * 100) / 100;
          ...
          for (index = 0; index < content.data.length; ++index) {
            ...
            if (content.fillColor != null) {
              if (Array.isArray(content.fillColor))
                fillColorForIndex = ctx.fillStyle = content.fillColor[index];
              else
                ctx.fillStyle = content.fillColor;
            } else
              ctx.fillStyle = options.fillColorBars;
            if (content.strokeColor != null) {
              if (Array.isArray(content.strokeColor))
                strokeColorForIndex = ctx.strokeStyle = content.strokeColor[index];
              else
                ctx.strokeStyle = content.strokeColor;
            } else
              ctx.strokeStyle = options.strokeColorBars;
            ...
            if (vIsArr && options.barStyle === "stacked") {
              ...
              for (var drawIndex = 0; drawIndex < v.length; ++drawIndex) {
                ...
                if (content.hints != null && content.hints[index] != null && (hint = content.hints[index][drawIndex]) != null) {
                  this.mouseListeners.push((function(index3, drawIndex2, hint2, sx, sy, ex, ey, x2, y2) {
                    var minX = Math.min(sx, ex), maxX = Math.max(sx, ex);
                    var minY = Math.min(sy, ey), maxY = Math.max(sy, ey);
                    if (x2 < minX || x2 > maxX || y2 < minY || y2 > maxY)
                      return null;
                    return { index: index3, drawIndex: drawIndex2, rect: { left: minX, right: maxX, top: minY, bottom: maxY }, text: hint2.split("\n") };
              ...
              if (vIsArr) {
                ...
                lastData = nLData;
                if (lDu != null && lDu.color != lastData[0].color)
                  this.fillRegions.push({
                    x: lastData[0].x,
                    y: lastData[0].y,
                    prev: lDu.color,
                ...
                if (lDu != null && lDu.color != lastData.color)
                  this.fillRegions.push({
                    x: lastData.x,
                    y: lastData.y,
                    prev: lDu.color,
              ...
              if (content.hints != null && (hint = content.hints[index]) != null) {
                this.mouseListeners.push((function(index3, hint2, sx, sy, ex, ey, x2, y2) {
                  var minX = Math.min(sx, ex), maxX = Math.max(sx, ex);
                  var minY = Math.min(sy, ey), maxY = Math.max(sy, ey);
                  if (x2 < minX || x2 > maxX || y2 < minY || y2 > maxY)
                    return null;
                  return { index: index3, drawIndex, rect: { left: minX, right: maxX, top: minY, bottom: maxY }, text: hint2.split("\n") };
